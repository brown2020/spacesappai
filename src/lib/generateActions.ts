...
export async function generateSummary(document: string, language: string) {
  const systemPrompt =
  ...
  return generateResponse(systemPrompt, userPrompt, "gpt-4o");
}

export async function generateAnswer(document: string, question: string) {
  const systemPrompt =
